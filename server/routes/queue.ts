...
// GET /api/queue/today
queueRouter.get('/today', authenticateToken, (req: AuthenticatedRequest, res: Response): void => {
  const { doctorId, status } = req.query as { doctorId?: string; status?: string };
  const items = db.getTodayQueue({ doctorId, status });
  res.json(items);
});

// GET /api/queue/my-status (Patient live queue view)
queueRouter.get('/my-status', authenticateToken, (req: AuthenticatedRequest, res: Response): void => {
  const user = req.user!;
  ...
  const status = db.getPatientQueueStatus(patient.id);
  res.json(status);
});

// POST /api/queue/check-in
queueRouter.post('/check-in', authenticateToken, (req: AuthenticatedRequest, res: Response): void => {
  try {
    const user = req.user!;
    ...
    if (user.role === 'PATIENT') {
      ...
      const apt = db.getAppointmentById(appointment_id);
    ...
    const result = db.checkInPatient({
      appointment_id,
      patient_id,
      doctor_id,
      checkedInBy: {
...
queueRouter.post('/call-next', authenticateToken, requireRoles('DOCTOR', 'ADMIN'), (req: AuthenticatedRequest, res: Response): void => {
  ...
  const result = db.callNext(doctor_id, {
    id: user.id,
    email: user.email,
    role: user.role,
  });
...
queueRouter.post('/:id/recall', authenticateToken, requireRoles('DOCTOR', 'ADMIN'), (req: AuthenticatedRequest, res: Response): void => {
  const result = db.recallPatient(req.params.id, {
    id: req.user!.id,
...
queueRouter.post('/:id/skip', authenticateToken, requireRoles('DOCTOR', 'ADMIN'), (req: AuthenticatedRequest, res: Response): void => {
  const result = db.skipPatient(req.params.id, {
    id: req.user!.id,
    email: req.user!.email,
    role: req.user!.role,
  });
...
queueRouter.post('/:id/start-consultation', authenticateToken, requireRoles('DOCTOR', 'ADMIN'), (req: AuthenticatedRequest, res: Response): void => {
  const result = db.startConsultation(req.params.id, {
    id: req.user!.id,
    email: req.user!.email,
    role: req.user!.role,
  });
...
queueRouter.post('/:id/complete', authenticateToken, requireRoles('DOCTOR', 'ADMIN'), (req: AuthenticatedRequest, res: Response): void => {
  ...
  const result = db.completeConsultation(
    req.params.id,
    {
      chief_complaint,
      symptoms: symptoms || '',
...
queueRouter.post('/:id/no-show', authenticateToken, requireRoles('DOCTOR', 'ADMIN'), (req: AuthenticatedRequest, res: Response): void => {
  const result = db.markNoShow(req.params.id, {
    id: req.user!.id,
    email: req.user!.email,
    role: req.user!.role,
  });
